<template>
  ...
  <div class="menu_right" :class="sysCloseMenu">
    ...
    <div class="listP" v-show="!$store.getters.phone">
      <div class="menu_head" >
      ...
      <div class="list">
        <el-menu
          router
          :default-active="defaultActive"
          :unique-opened="uniqueOpened?true:false"
...
<script>
  export default {
    ...
    props:["menuListData","uniqueOpened"],
    //方法
    methods:{
      menuPhoneClick(){
        this.menuPhone = this.menuPhone ? false : true;
      },
      menuStateHandle(){
        var _this = this;
        if(_this.isCollapse){
      ...
      changePhone(){
        ...
        if(that.screenWidth<700){
          ...
          that.$store.commit("MENUSTATE", "phone");
          this.sysCloseMenu="closeMenuPhone";
        }else {
          that.isOpen=false;
          that.phone=false;
          this.sysCloseMenu="";
